...
Component({
    ...
    methods: {
        ...
        setContent (html, append, _watch) {
                ...
                if (html.length && html[0].PoweredBy != "Parser") {
                    ...
                    (function f (ns) {
                        for (var i = 0, n; (n = ns[i]); i++) {
                            ...
                            n.attrs = n.attrs || {};
                            for (var key in n.attrs) if (typeof n.attrs[key] != "string") n.attrs[key] = n.attrs[key].toString();
                            parser.matchAttr(n);
                            if (n.children) {
                                parser.STACK.push(n);
                                f(n.children);
            ...
            } else return console.warn("错误的 html 类型：" + typeof html);
            if (append) {
                this._refresh = true;
        ...
        preLoad (html, num) {
            ...
            else if (this.data.imgs.length < 15) imgs = this.data.imgs.concat(this._wait.splice(0, 15 - this.data.imgs.length));
            imgs &&
                this.setData({
                    imgs
                });
